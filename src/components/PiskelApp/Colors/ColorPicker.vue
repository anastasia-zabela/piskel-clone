...
export default {
  ...
  computed: {
    ...
    currentColor() {
      const [h, s, l] = this.colorInHsl;
      return `hsl(${h}, ${s}%, ${l}%)`;
    },
    colorInHex() {
      return this.rgbToHex();
    },
    changeColorFormatButton() {
  ...
  methods: {
    ...
    createShadeSpectrum() {
      const spectrumCanvas = document.querySelector('.spectrum-map__spectrum-canvas');
    ...
    getSpectrumColor(e) {
      const cursor = document.querySelector('.spectrum-map__color-cursor');
      const spectrumCanvas = document.querySelector('.spectrum-map__spectrum-canvas');
      const ctxSpectrum = spectrumCanvas.getContext('2d');
      ...
      if (x > spectrumRect.width) { x = spectrumRect.width - 1; }
      if (x < 0) { x = 0; }
    ...
    getHueColor(e) {
      ...
      if (y > hueRect.height) { y = hueRect.height - 1; }
      if (y < 0) { y = 1; }
  ...
  mounted() {
    this.colorInRgb = this.color.match(/\d+/g);
